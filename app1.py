        ...
        with st.chat_message("assistant"):
            with st.spinner("관련 안전 규정을 검토 중입니다..."):
                try:
                    ...
                    st.error(err_msg)
                    st.session_state.qa_messages.append({"role": "assistant", "content": err_msg})# -*- coding: utf-8 -*-
import streamlit as st
from streamlit_autorefresh import st_autorefresh
from google import genai
import gspread
...
# ==========================================
nav_col1, nav_space, nav_col2, nav_col3, nav_col4, nav_col5, nav_col6, nav_col7, nav_col8 = st.columns([2, 1, 1, 1, 1, 1, 1, 1, 1])

with nav_col1:
    # 📌 col1 안에서 다시 작은 가로 칸을 2개로 쪼갭니다 (로고용, 글자용)
    logo_col, text_col = st.columns([0.2, 0.5])
    ...
    with text_col:
        ...
        st.markdown("""
            <div style="padding-top: 6px;">
                <span style="font-size: 2.2rem; font-weight: 600; color: #1F2937; letter-spacing: 1px;">한국환경공단</span>
            </div>
        """, unsafe_allow_html=True)
